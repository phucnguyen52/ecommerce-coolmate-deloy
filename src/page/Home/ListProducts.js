...
const ListProducts = () => {
    ...
    return (
        <>
            <div className="mb-20">
                <div className="mb-5 ml-4 flex gap-6">
                    <button
                        className={`${option === 'new' ? `bg-black text-white` : 'border border-black bg-white text-black'} rounded-3xl px-6 py-2 text-lg font-bold  `}
                        onClick={() => handleClick('new')}
                    >
                        Sản phẩm mới
                    </button>
                    <button
                        className={`${option === 'price' ? `bg-black text-white` : 'border border-black bg-white text-black'} rounded-3xl px-6 py-2 text-lg font-bold  `}
                ...
                <div className="relative ml-4 overflow-hidden">
                        ...
                        {data &&
                            data.map((product) => (
                                <div
                                    key={product.id}
                                    className=" mr-8 "
                                    style={{ minWidth: `calc((100vw - 2rem*${slide} - 1rem)/${slide})` }}
                                >
                    ...
                    <div
                        ...
                        className="absolute left-0 top-1/2 rounded-full bg-black bg-opacity-15 p-3 text-2xl"
                    >
                        <HiArrowLeft />
                    </div>
                    <div
                        onClick={nextSlide}
                        className="absolute right-4 top-1/2 rounded-full bg-black bg-opacity-15 p-3 text-2xl"
                    >
                        <HiArrowRight />
                    </div>
                </div>
